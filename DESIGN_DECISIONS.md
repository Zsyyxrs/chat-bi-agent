...
**新的下一步**：
1. **扩题量**：34 题里指标型 20 题，`routing_accuracy` 的分母仍偏小
   （recall 0.75 = 15/20，少一题就跳 5 个点）。目标 60+ 题
2. **评分器看不见语义不忠实**：mr_n12 被误路由时只扣 0.075，是逐题读 SQL 才发现的。
   需要一个直接比对结果集的评分维度，而不是只比 SQL 文本特征
3. **阈值随模型漂移**：0.63 绑定当前 embedding 模型。换 embedding 必须重扫
   （只 embed 问题即可，几十秒，与实跑 cosine 偏差 0.000218）
