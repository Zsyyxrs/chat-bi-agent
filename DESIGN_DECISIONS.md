...
截至本次提交，34 题新标尺上有一轮**完整**的三臂数据，跑在 commit `82bf5e4`
（即 Top-N 拒绝规则 `a9c3b83` **之前**）：

| 指标 | Baseline | Router t=0.70 | Router t=0.63 |
|---|---:|---:|---:|
| avg_score | 0.9373 | 0.9355 | **0.9498** |
| passed | 32/34 | 31/34 | 32/34 |
| metric_hit_rate | — | 0.2647 | 0.4706 |
| 路由 precision | — | **1.000** | 0.9375 |
| 路由 recall | — | 0.45 | **0.75** |
| 路由 F1 | — | 0.6207 | **0.8333** |
| prefilter 误触 | — | 1 | 5 |

关键逐题事实：
- t=0.70 下 **9/9 走语义层的题得分与 baseline 完全相同**，零假阳性路由
- t=0.63 下 16 题走语义层，其中 **2 题比 baseline 更好**
  （mr_m03 +0.217、mr_m15 +0.250——governed 模板赢过 LLM 手写 SQL），
  13 题持平，1 题更差（mr_n12，即上文 Top-N 漏洞，已由 `a9c3b83` 修掉）
- 4 次 prefilter 误触全部安全回退，得分未受影响

**为什么标"待回填"**：`a9c3b83`（Top-N 拒绝）之后的确认跑尚未完成——两轮跑批
分别毁于 DashScope 免费额度耗尽与 `read timeout=300`（后者根因见 `38aada4`：
超时参数名写错，93c5b96 从未生效）。上表因此是**保守下界**：mr_n12 是 t=0.63
唯一的假阳性，修复后其 precision 应升至 1.000、F1 至 0.857，但**这是推算而非实测**。

**默认阈值维持 0.70 未改**：0.63 的优势要等实测确认后再动代码默认值。
