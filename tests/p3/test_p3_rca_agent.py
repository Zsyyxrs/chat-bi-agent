...
def _good_fact_anchor_p1(qid: str) -> FakeP1Result:
    sql = (
        "SELECT SUM(balance), prior_balance FROM t"
...
def test_happy_path_returns_full_report(fake_events_dir: Path):
    ...
    llm.chat.side_effect = [
        ...
        _content("narrative including BR_CITY_0006 HIGH_NET_WORTH 安鑫 anxin_90_expire"),
    ]
    agent = P3RootCauseAnalysisAgent(
        p1_agent=p1, llm_client=llm, events_dir=fake_events_dir
    )
