...
def test_returns_none_when_catalog_missing(tmp_path):
    with patch.object(wiring, "METRICS_CATALOG_PATH", tmp_path / "nope.yaml"):
        assert p1_nl2sql._build_metric_router_if_available() is None


def test_builds_router_when_catalog_present():
    with (
        patch.object(wiring.qwen_client, "embed", side_effect=lambda t: [[1.0, 0.0] for _ in t]),
        patch.object(wiring, "SQLExecutor", MagicMock()),
    ):
...
def test_degrades_to_none_when_embedding_fails():
    ...
    with (
        patch.object(wiring.qwen_client, "embed", side_effect=RuntimeError("dashscope down")),
        patch.object(wiring, "SQLExecutor", MagicMock()),
    ):
        assert p1_nl2sql._build_metric_router_if_available() is None
...
def test_degrades_to_none_when_catalog_malformed(tmp_path):
    ...
    with patch.object(wiring, "METRICS_CATALOG_PATH", bad):
        assert p1_nl2sql._build_metric_router_if_available() is None
