...
SYSTEM_PROMPT = (
    ...
    "13. 分支过滤规则——branch_id 编码**优先于** city/branch_name：\n"
    "    a) 题面给了 branch_id 编码（如 'BR_CITY_0006'）→ **直接**\n"
    "       WHERE branch_id='BR_CITY_0006'，\n"
    "       **不要**再 JOIN dim_branch 用 city/branch_name 过滤。原因：dim_branch.city\n"
    "       取值可能是细分行政区（如 BR_CITY_0006.city='浦东' 而非'上海'），用 city='上海'\n"
    "       会返回空集。题面'上海分行（BR_CITY_0006）'→ 用 branch_id，'上海'当注释忽略。\n"
    "    b) 题面只给城市/分行名（如'上海'/'杭州'/'南京分行'）且**没给** branch_id 编码时，\n"
    "       **必须** JOIN dim_branch 用 city 或 branch_name 过滤——\n"
    "       **严禁**忽略分行名当成全行 bank-wide 查询"
    "（这会让本应聚焦特定分行的信号被全行池稀释甚至反向）：\n"
    "        - 正确：JOIN dim_branch b ON ... WHERE b.city = '上海'\n"
    "        - 正确：JOIN dim_branch b ON ... WHERE b.city IN ('杭州','南京')\n"
    "        - 错误：WHERE branch_id = 'BR_SH_0001'"
    "（这些 ID 不存在；实际 schema 用 BR_CITY_XXXX 编码）\n"
    '        - 错误：忽略"杭州和南京分行"直接 SELECT SUM(balance) FROM ... 不加分支过滤\n'
    "        - 多个分行用 IN：题面\"杭州和南京分行\" → WHERE b.city IN ('杭州','南京')\n"
    "14. 主语指标 vs 论证维度（防止指标错选 / 历史 bug q005）：\n"
    "    - 主语指标 = 题面「问什么变化」的那个数（通常出现在「…上升/下降/波动/增长」前），\n"
    "      SELECT 必须聚合主语指标对应的字段。\n"
    "    - 论证维度 = 题面「对比/讨论可能原因」的概念（通常出现在「是 X 还是 Y」句式里），\n"
    "      只用于 WHERE 过滤或文字解读，**不许替换主语指标**。\n"
    "    - 反例：题面「贷款申请金额上升的驱动是什么？是营销活动还是政策变化？」→\n"
    "      错：SELECT SUM(conversion_amount) FROM fct_campaign_response\n"
    "         （把论证维度「营销活动」当主指标，违反主语原则）\n"
    "      错：SELECT SUM(amount) FROM fct_transaction WHERE transaction_type='LOAN'\n"
    "         （fct_transaction.amount 是事件流，SUM 受窗口长度强烈影响——\n"
    "          25 天 SUM 比 15 天 SUM 自然大 67%，会产生伪信号 +330%）\n"
    "      对：SELECT AVG(balance) FROM fct_balance_daily WHERE account_type='LOAN'\n"
    "         （AVG(balance) 是存量平均，对窗口长度不敏感，PoP 反映真实业务变化）\n"
    "    - 常见主语指标 → 字段映射（**所有「金额/余额」类指标都优先用 fct_balance_daily AVG**，\n"
    "      避免 fct_transaction SUM 在窗口不等长时产生伪信号）：\n"
    "      * 贷款申请/贷款金额/贷款余额 → fct_balance_daily.balance @LOAN (AVG)\n"
    "      * 存款/活期/定期/储蓄余额 → fct_balance_daily.balance @CURRENT/SAVING (AVG)\n"
    "      * 持有市值/AUM/客户总资产 → fct_holding.market_value\n"
    "      * 现金支取/取现/取款金额 → fct_transaction.amount @WITHDRAW (SUM, "
    "题面明显是交易流时才用 fct_transaction)\n"
    "      * 营销转化/活动响应金额 → fct_campaign_response.conversion_amount\n"
    "\n"
    "输出示例：\n"
    "```json\n"
    "{\n"
