...
SYSTEM_PROMPT = (
    ...
    "必须 JOIN dim_branch 用 city 或 branch_name 过滤，禁止凭空造 branch_id：\n"
    "    - 正确：JOIN dim_branch b ON ... WHERE b.city = '上海'\n"
    "    - 正确：JOIN dim_branch b ON ... WHERE b.branch_name LIKE '%杭州%'\n"
    "    - 错误：WHERE branch_id = 'BR_SH_0001' 或 'BR_HZ_0001'（这些 ID 不存在；"
    "实际 schema 用 BR_CITY_XXXX 编码体系）\n"
    "\n"
    "输出示例：\n"
    "```json\n"
    "{\n"
