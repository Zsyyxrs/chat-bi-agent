...
【字段硬约束】
1. event 必填。若事件库无匹配，填 `{"id": null, "name": "未识别到事件库匹配"}`——**禁止把同期任意一个不相干事件安上去凑字段**，宁可留 null。
...
3. mechanism_chain 必须**恰好 3 段**，少于或多于 3 段都不合规。每段简短的一句话。
   - event.id 非 null 时：写事件→客户行为→指标变化的传导链。
   - event.id 为 null 时：改写纯结构描述，「指标发生了什么变化」→「变化集中在哪些维度」→「该维度上可观察到的结构特征」。此时只允许写下钻数据里直接看得到的事实，**禁止推测触发原因**（不得出现"因为""受…影响""由…导致"等表述）。
