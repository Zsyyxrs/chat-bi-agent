...
class RCAEvaluator:
    ...
    _JUDGE_SYSTEM_PROMPT = (
        ...
        "  2. quantification: agent 是否给出方向一致、幅度接近的量化数字（详见下方专门规则）\n"
        "  3. mechanism: agent 是否说清因果链（事件→中间环节→指标变化）\n"
        "  4. scope: agent 是否准确界定影响范围（分行/客户层级/产品维度等）\n\n"
        "===== quantification 判分专门规则（机械执行，不要主观加严）=====\n"
        "若输入包含【期望关键指标】，对其中每条 metric 按以下流程判：\n"
        "  Step 1 - 方向：narrative 报的指标变化方向是否与 ground truth `direction` 一致？\n"
        "  Step 2 - 幅度：narrative 给的 PoP/环比绝对值 vs ground truth `magnitude` 绝对值，"
        "差距比例 = |narr - mag| / |mag|。\n"
        "  打分：\n"
        "    - 方向一致 且 差距比例 ≤ 50%  → quantification = 1.0\n"
        "    - 方向一致 且 50% < 差距比例 ≤ 100% → quantification = 0.5\n"
        "    - 方向反 或 差距比例 > 100% 或 narrative 完全无数字 → quantification = 0.0\n"
        "  Metric 名等价规则（极重要，违反会错判）：\n"
        "    - 英文 metric 名与中文自然表述视为等价。\n"
        "      例：wealth_category_total_market_value ≡「理财产品持有余额」"
        "≡「WEALTH 大类持有市值」\n"
        "      例：average_daily_balance ≡「日均余额」\n"
        "      例：saving_deposit_balance ≡「定期存款余额」\n"
        "      例：cash_withdrawal_amount ≡「现金支取金额」\n"
        "      例：AUM ≡「资产管理规模」≡「客户总资产」\n"
        "    - 只要 narrative 谈的是同一类指标（如同属余额/市值/转化金额/支取金额），"
        "就视为 metric 名命中，不要因翻译措辞差异扣 quantification 分。\n"
        "  多条 metric 时取平均分。\n"
        "===== quantification 规则结束 =====\n\n"
        "===== mechanism 判分专门规则（机械执行，不要主观加严）=====\n"
        "检查 agent narrative 是否清晰呈现「触发事件 → 客户/资金行为 → 指标变化」三段因果链。\n"
        "每一段只要在 narrative 里能找到对应表达即视为命中，不要求展开细节、不要求"
        "具体响应时延 / 客户心理 / 政策传导步骤 / 渠道命名。\n"
        "  打分：\n"
        "    - 三段都点到（事件 + 中间行为 + 指标结果） → mechanism = 1.0\n"
        "    - 命中其中两段 → mechanism = 0.5\n"
        "    - 只命中一段（如只提事件没说行为，或只说指标变化没说因果） → mechanism = 0.25\n"
        "    - 完全无机制叙述（只罗列数字 / 仅事件名 + 数字） → mechanism = 0.0\n"
        "  命中宽容：\n"
        "    - 「客户响应/资金流入/资金赎回/转入/购买」等都算『中间行为』。\n"
        "    - 「营销活动上线/产品到期/政策发布/节日来临」等都算『触发事件』。\n"
        "    - 「余额上升/下降/增长/流出」等都算『指标变化』。\n"
        "===== mechanism 规则结束 =====\n\n"
        ...
        "评分校准（针对 event/scope 两维；quantification 与 mechanism 走上面专门规则）：\n"
        "- 完全命中 → 1.0\n"
        "- 方向对但细节缺 → 0.5\n"
        "- 完全没提或方向错 → 0.0\n"
