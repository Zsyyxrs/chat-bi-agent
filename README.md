...
| **P2 多步分析** | 3 / 8 | 0 | **0.601** | 2026-08-17 | 4 维计分（洞察 35% + 步骤 25% + 多指标 15% + rubric LLM judge 25%）；推理/业务相关降为诊断 |
...
**P2 的 0.601 是本项目唯一持续「往下修」的分数**，因为此前的高分有相当部分是白送的。
2026-08-15/17 分三步修：
...
| ③ 补 rubric LLM judge | 照 P3 做 4 维 G-Eval，每维锚在**本题** YAML 字段上，占 25% | **0.601** | **0/3** |
...
**judge 挖出的实质缺陷是 `quantification`：三题 0.50 / 0.00 / 0.00。** agent 基本不报
与期望基准可比的数字——此前没有任何维度看得见这件事（`insight_accuracy` 算内容词召回，
说到「增长」就算命中，不管报的是 +25% 还是 +3%）。

**0/3 通过要读作「三题都在 0.7 以下」，但别把它当精确刻度**：q001 两次跑分别落在
0.700 与 0.679，跨在及格线两侧，`passed` 因此在 1/3 与 0/3 之间摆动。表里取的是
`commit_dirty=false` 那一份。

**仍未解决，且都卡在同一件事上——`n=3`**：

- `multi_metric_coverage` 在这 3 题上全满分，候选词（率/增长/金额/客户/流）太通用。它的
  意图与 judge 的 `step_fidelity` 高度重叠（后者拿 `analysis_steps` 当锚，判得更细），
  所以更一致的动作可能是**删掉它**而非调词表——但它在 q001 上取过 0.500，不是纯常数，
  3 题的证据不足以下这个结论。
