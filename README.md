...
**2026-09-08 重述：0.965 → 0.977。** 查出评分器的 `table_score` 把 CTE 名当成了表名
（正则 `(?:FROM|JOIN)\s+(\w+)` 会匹配 `FROM txn_agg` 这种对 CTE 的引用）。这不是随机
噪声而是**系统性偏置**，且不对称：gold 惯用派生子查询（`FROM (` 不匹配），生成 SQL 用
`WITH` 就中招。q008 的表选择本应满分却被算成 0.5，白扣 0.1。修复后用
[`scripts/replay_p1_scoring.py`](./scripts/replay_p1_scoring.py) 离线重打分（**零 LLM 调用**，
SQL 取自原产物重打真库），8 题 avg 0.9646 → **0.9771**，其余七题一分未动。

**连带修正一条此前的判断**：原先此处写「同一配置反复跑 avg 落在 0.965~0.977，差异全部
来自 q008 的跑间抖动」。重述后 2026-08-14 与 08-15 两次跑**都是 0.9771**——那个「回退」
根本不存在，它是模型在两次之间把 q008 换成了 CTE 写法，而评分器为这个**等价**写法扣了
0.1。真实的跑间变化是「SQL 形式变了」，不是「质量降了」。这恰好也是语义层「确定性」
价值的另一个切面：模型给出两条等价 SQL，我的评分器却给了两个分数。

跑间抖动本身仍然存在（q008 是这 8 题里最复杂的：双窗口 + 条件聚合 + 变化率 + Top-N），
判断优劣仍应看逐题比较而非 avg 上的零点几个百分点——详见
...
修完后**6 题口径回到 1.000**（可复现），8 题全量 0.977（2026-09-08 重述值，原报 0.965），
8/8 全部及格。差额来自 q005（agent 漏了「定期存款」约束，真扣）。
