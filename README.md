...
| **P2 多步分析** | 3 / 8 | 2 | **0.655** | 2026-08-17 | 3 维计分（洞察 40% + 步骤完整 30% + 多指标 30%）；推理/业务相关降为诊断 |
...
**P2 的 0.655 是本项目唯一「往下修」的分数**，因为此前的高分有相当部分是白送的。
2026-08-15/17 分两步修：先修洞察维（对中文按空格切词，等于没分词）与指标覆盖维
（按**单个字**匹配，'长'/'户' 这类字在银行叙述里几乎必然出现），再把推理质量与
业务相关性移出总分——它们**没有可比对的对象**，判据只能是数关键词，实测恒等 1.000，
合计 35% 权重是常数而非测量。0.798 → 0.655、3/3 → 2/3 通过，就是挤掉水分的结果。
详见 [ADR-015](./DESIGN_DECISIONS.md#adr-015)。

**仍未解决**：`multi_metric_coverage` 在 3 题里仍有 2 题满分——整词匹配修掉了最离谱
的误判，但候选词（率/增长/金额/客户/流）本身太通用。要真正度量推理与业务相关性，
需照 P3 的 `_llm_judge_conclusion` 补 rubric LLM judge，尚未做。
