...
| **P2 多步分析** | 3 / 8 | 3 | **0.798** | 2026-08-15 | 5 维 rubric（步骤完整 + 多指标 + 洞察 + 推理 + 业务相关） |
...
**P2 的 0.798 只是「不再明显错」，尚不足以当能力指标**：五维里有三维
（多指标覆盖 / 推理质量 / 业务相关性，合计 **55% 权重**）在每一题上都恒等于 1.000
——判据是数关键词个数、达到 4~5 个即满分，任何通顺的中文分析都会自动拿满，不提供
任何区分度。2026-08-15 修好了权重最高的洞察维（此前对中文按空格切词，等于没分词，
详见 [ADR-015](./DESIGN_DECISIONS.md#adr-015)），0.740 → 0.798 全部来自该维；
另外三维要不要收紧是尚未做的决定。
